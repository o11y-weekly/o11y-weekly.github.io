...
### Alternatives
An alternative of instrumented code can be using external distributed tracing to decorate legacy app by using a proxy. [Envoy](https://www.envoyproxy.io/docs/envoy/latest/start/sandboxes/opentelemetry) can be used to support opentelemetry widely on a huge legacy to offer the distributed tracing between application without having deep details. It can be a good start to have the global picture to find out which service is the less effective first without too much integration effort.
